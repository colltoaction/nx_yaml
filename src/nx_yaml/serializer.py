...
class NxSerializer:
    ...
    def __init__(self, stream):
        ...
        # event_get(self.event, "encoding") = encoding
        ...
        # # Formatting details.
        # event_get(self.event, "canonical") = canonical
        # self.allow_unicode = allow_unicode
        # 2 = 2
        # if indent and 1 < indent < 10:
        #     2 = indent
        # 80 = 80
        # if width and width > 2*2:
        #     80 = width
        # '\n' = '\n'
        # if line_break in ['\r', '\n', '\r\n']:
        #     '\n' = line_break
    ...
    def emit_node(self, node: HyperGraph, parent, index):
        ...
        if k == "stream":
            ...
            self.emit_document(node, parent, index)
        elif k == "alias":
            self.emit_alias(node, parent, index)
        elif k == "scalar":
            self.emit_scalar(node, parent, index)
        elif k == "sequence":
            self.emit_sequence(node, parent, index)
        elif k == "mapping":
            self.emit_mapping(node, parent, index)
        else:
            raise EmitterError(f"unknown node kind {k}")

    def emit_alias(self, node, parent, index):
    ...
    def emit(self, event_name, node, parent, index):
        self.events.append((event_name, node, parent, index))
    ...
    def need_more_events(self):
        if not self.events:
            return True
        event = self.events[0]
        if event[0] == "DocumentStartEvent":
            return self.need_events(1)
        elif event[0] == "SequenceStartEvent":
            return self.need_events(2)
        elif event[0] == "MappingStartEvent":
    ...
    def need_events(self, count):
        ...
        for event in self.events[1:]:
            if event[0] in ("DocumentStartEvent", "SequenceStartEvent", "MappingStartEvent"):
                level += 1
            elif event[0] in ("DocumentEndEvent", "SequenceEndEvent", "MappingEndEvent"):
                level -= 1
            elif event[0] == "StreamEndEvent":
                level = -1
            if level < 0:
                return False
    ...
    def expect_stream_start(self):
        if self.event[0] == "StreamStartEvent":
    ...
    def expect_nothing(self):
        raise EmitterError("expected nothing, but got %s" % self.event[0])
    ...
    def expect_document_start(self, first=False):
        if self.event[0] == "DocumentStartEvent":
            if (event_get(self.event, "version") or event_get(self.event, "tags")) and self.open_ended:
                self.write_indicator('...', True)
                self.write_indent()
            if event_get(self.event, "version"):
                version_text = self.prepare_version(event_get(self.event, "version"))
                self.write_version_directive(version_text)
            self.tag_prefixes = self.DEFAULT_TAG_PREFIXES.copy()
            if event_get(self.event, "tags"):
                handles = sorted(event_get(self.event, "tags").keys())
                for handle in handles:
                    prefix = event_get(self.event, "tags")[handle]
                    self.tag_prefixes[prefix] = handle
                    handle_text = self.prepare_tag_handle(handle)
                    prefix_text = self.prepare_tag_prefix(prefix)
                    self.write_tag_directive(handle_text, prefix_text)
            implicit = (first and not event_get(self.event, "explicit") and not event_get(self.event, "canonical")
                    and not event_get(self.event, "version") and not event_get(self.event, "tags")
                    and not self.check_empty_document())
            if not implicit:
                self.write_indent()
                self.write_indicator('---', True)
                if event_get(self.event, "canonical"):
                    self.write_indent()
            self.state = self.expect_document_root
        elif self.event[0] == "StreamEndEvent":
            if self.open_ended:
                self.write_indicator('...', True)
                self.write_indent()
    ...
    def expect_document_end(self):
        if self.event[0] == "DocumentEndEvent":
            self.write_indent()
    ...
    def expect_node(self, root=False, sequence=False, mapping=False,
        ...
        if root and self.event[0] in ("DocumentEndEvent", "StreamEndEvent"):
            # the empty file
            pass
        elif self.event[0] == "AliasEvent":
            self.expect_alias()
        elif self.event[0] == "DocumentStartEvent":
            self.expect_document_start()
        elif self.event[0] in ("ScalarEvent", "SequenceStartEvent", "MappingStartEvent"):
            self.process_anchor('&')
            self.process_tag()
    ...
    def expect_first_flow_sequence_item(self):
        if self.event[0] == "SequenceEndEvent":
            self.indent = self.indents.pop()
            self.flow_level -= 1
            self.write_indicator(']', False)
            self.state = self.states.pop()
        else:
            if event_get(self.event, "indent"):
                self.write_indent()
    ...
    def expect_first_flow_mapping_key(self):
        if self.event[0] == "MappingEndEvent":
            self.indent = self.indents.pop()
            self.flow_level -= 1
            self.write_indicator('}', False)
            self.state = self.states.pop()
            ...
            if event_get(self.event, "indent"):
                self.write_indent()
            if not event_get(self.event, "canonical") and self.check_simple_key():
    ...
    def expect_block_sequence_item(self, first=False):
        if not first and self.event[0] == "SequenceEndEvent":
            self.indent = self.indents.pop()
            self.state = self.states.pop()
        else:
            self.write_indent()
    ...
    def expect_block_mapping_key(self, first=False):
        if not first and self.event[0] == "MappingEndEvent":
            self.indent = self.indents.pop()
            self.state = self.states.pop()
        else:
            self.write_indent()
    ...
    def check_empty_sequence(self):
        return (self.event[0] == "SequenceStartEvent" and self.events
                and self.events[0][0] == "SequenceEndEvent")

    def check_empty_mapping(self):
        return (self.event[0] == "MappingStartEvent" and self.events
                and self.events[0][0] == "MappingEndEvent")
    ...
    def write_stream_start(self):
        # Write BOM if needed.
        encoding = event_get(self.event, "encoding")
        if encoding and encoding.startswith('utf-16'):
            self.stream.write('\uFEFF'.encode(encoding))
    ...
    def write_indicator(self, indicator, need_whitespace,
        ...
        self.open_ended = False
        if event_get(self.event, "encoding"):
            data = data.encode(event_get(self.event, "encoding"))
    ...
    def write_single_quoted(self, text, split=True):
        ...
        while end <= len(text):
            ...
            if spaces:
                if ch is None or ch != ' ':
                    if start+1 == end and event_get(self.event, "indent") and split   \
    ...
    def write_double_quoted(self, text, split=True):
        ...
        while end <= len(text):
            ...
            if 0 < end < len(text)-1 and (ch == ' ' or start >= end)    \
                    and self.column+(end-start) > 80 and split:
                data = text[start:end]+'\\'
                if start < end:
                    start = end
                self.column += len(data)
    ...
    def write_folded(self, text):
        ...
        while end <= len(text):
            ...
            if breaks:
                ...
                if ch != ' ':
                    if start+1 == end and event_get(self.event, "indent"):
                        self.write_indent()
                    else:
                        data = text[start:end]
                        self.column += len(data)
